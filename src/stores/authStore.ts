    ...
    persist(
      (set, get) => ({
        ...
        isTokenExpired: (): boolean => {
          ...
          return (
            Date.now() >= authData.expiresAt - AUTH_CONFIG.TOKEN_EXPIRY_BUFFER
          );
        },
      ...
      }),
      {
        ...
        onRehydrateStorage: () => (state) => {
          ...
          if (state?.authData) {
            ...
            if (
              state.authData.isOAuth &&
              state.authData.expiresAt &&
              now >= state.authData.expiresAt
            ) {
...
export const useHasValidAuth = () =>
  useAuthStore((state) => state.hasValidAuth());
