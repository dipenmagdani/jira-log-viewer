 ...
 */
const fetchIssuesByProject = async (
 ...
 */
const searchIssuesByText = async (
  query: string,
  projectKeys?: string[]
): Promise<JiraIssue[]> => {
  let jql = `text ~ "${query}"`;
...
const fetchIssueTypes = async (): Promise<JiraIssueType[]> => {
  const response =
    await apiClient.get<ApiResponse<JiraIssueType[]>>("/api/issue-types");
...
export const useIssueSummary = (issueKey: string): string => {
  ...
  const cachedIssue = queryClient.getQueryData(
    queryKeys.issues.detail(issueKey)
...
export const useIssueFromCache = (issueKey: string): JiraIssue | undefined => {
  ...
  return queryClient.getQueryData(queryKeys.issues.detail(issueKey)) as
    | JiraIssue
    | undefined;
};
...
export const buildJQL = (conditions: Record<string, any>): string => {
  ...
  if (conditions.project) {
    if (Array.isArray(conditions.project)) {
      parts.push(
        `project IN (${conditions.project.map((p: string) => `"${p}"`).join(", ")})`
      );
    } else {
  ...
  if (conditions.issueType) {
    if (Array.isArray(conditions.issueType)) {
      parts.push(
        `issueType IN (${conditions.issueType.map((t: string) => `"${t}"`).join(", ")})`
      );
    } else {
  ...
  if (conditions.status) {
    if (Array.isArray(conditions.status)) {
      parts.push(
        `status IN (${conditions.status.map((s: string) => `"${s}"`).join(", ")})`
      );
    } else {
