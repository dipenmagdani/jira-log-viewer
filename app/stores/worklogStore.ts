...
const applyFilters = (worklogs: JiraWorklog[], filters: WorklogFilter): JiraWorklog[] => {
  return worklogs.filter(worklog => {
    // Date range filter
    if (filters.dateRange.start || filters.dateRange.end) {
      const worklogDate = new Date(worklog.started);
      if (filters.dateRange.start && worklogDate < filters.dateRange.start) return false;
      if (filters.dateRange.end && worklogDate > filters.dateRange.end) return false;
    }
    ...
    if (filters.timeSpentRange.min !== null || filters.timeSpentRange.max !== null) {
      if (filters.timeSpentRange.min !== null && worklog.timeSpentSeconds < filters.timeSpentRange.min) return false;
      if (filters.timeSpentRange.max !== null && worklog.timeSpentSeconds > filters.timeSpentRange.max) return false;
    }
...
const applySorting = (worklogs: JiraWorklog[], sort: SortConfig): JiraWorklog[] => {
...
const calculateSummary = (worklogs: JiraWorklog[]): WorklogSummary => {
  ...
  worklogs.forEach(worklog => {
    ...
    issueMap.set(worklog.issue.key, currentIssueTime + worklog.timeSpentSeconds);
    
    // Count by author
    const currentAuthorTime = authorMap.get(worklog.author.accountId) || 0;
    authorMap.set(worklog.author.accountId, currentAuthorTime + worklog.timeSpentSeconds);
...
// Data selectors
export const useWorklogs = () => useWorklogStore((state) => state.filteredWorklogs);
export const useAllWorklogs = () => useWorklogStore((state) => state.worklogs);
export const useSelectedWorklog = () => useWorklogStore((state) => state.selectedWorklog);
export const useWorklogSummary = () => useWorklogStore((state) => state.summary);

// UI state selectors
export const useWorklogLoading = () => useWorklogStore((state) => ({
...
export const useWorklogFilters = () => useWorklogStore((state) => state.filters);
export const useWorklogPagination = () => useWorklogStore((state) => state.pagination);
export const useWorklogSort = () => useWorklogStore((state) => state.sort);

// Action selectors
export const useWorklogActions = () => useWorklogStore((state) => ({
  // Data actions
