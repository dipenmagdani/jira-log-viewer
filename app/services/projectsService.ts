 ...
 * Fetch all projects
 */
const fetchProjects = async (params: ProjectsListParams = {}): Promise<JiraProject[]> => {
...
export const useProjectName = (projectKey: string): string => {
  ...
  const cachedProjects = queryClient.getQueryData(queryKeys.projects.all()) as JiraProject[] | undefined;
 ...
 */
export const useProjectFromCache = (projectKey: string): JiraProject | undefined => {
  const queryClient = useQueryClient();
  
  // First try specific project cache
  const cachedProject = queryClient.getQueryData(queryKeys.projects.detail(projectKey)) as JiraProject | undefined;
