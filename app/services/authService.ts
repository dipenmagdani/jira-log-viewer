 ...
 */
const loginWithBasicAuth = async (credentials: LoginCredentials): Promise<LoginResponse> => {
 ...
 */
const loginWithOAuth = async (tokenData: OAuthTokenData): Promise<LoginResponse> => {
  const response = await apiClient.post<ApiResponse<LoginResponse>>(
    API_ENDPOINTS.INTERNAL.AUTH.LOGIN,
    {
      ...tokenData,
...
const getSession = async (): Promise<{ user: User; authData: AuthData }> => {
  const response = await apiClient.get<ApiResponse<{ user: User; authData: AuthData }>>(
...
const getUserProfile = async (): Promise<User> => {
  const response = await apiClient.get<ApiResponse<User>>(API_ENDPOINTS.INTERNAL.USER);
...
const testConnection = async (): Promise<{ success: boolean; message: string }> => {
  const response = await apiClient.get<ApiResponse<{ success: boolean; message: string }>>(
    '/api/test-connection'
  );
...
export const useBasicAuthLogin = (
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      localStorage.setItem(AUTH_CONFIG.STORAGE_KEYS.AUTH_DATA, JSON.stringify(data.authData));
    },
    onError: (error) => {
...
export const useRefreshTokens = (
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      const currentSession = queryClient.getQueryData(queryKeys.auth.session()) as any;
      if (currentSession) {
        ...
        localStorage.setItem(AUTH_CONFIG.STORAGE_KEYS.AUTH_DATA, JSON.stringify(updatedAuthData));
      }
    },
    onError: (error) => {
...
export const useIsAuthenticated = (): boolean => {
  ...
  if (!session?.authData) {
    ...
    try {
      ...
      if (storedAuth) {
        ...
        return !!(authData && (authData.accessToken || (authData.email && authData.apiToken)));
      }
    } catch (error) {
  ...
  if (session.authData.isOAuth && session.authData.expiresAt) {
    return Date.now() < session.authData.expiresAt - AUTH_CONFIG.TOKEN_EXPIRY_BUFFER;
  }
