 ...
 */
const fetchWorklogs = async (params: WorklogListParams = {}): Promise<WorklogListResponse> => {
 ...
 */
const fetchWorklogsByIssue = async (issueKey: string): Promise<JiraWorklog[]> => {
  const response = await apiClient.get<ApiResponse<JiraWorklog[]>>(
 ...
 */
const fetchWorklogAnalytics = async (params: WorklogAnalyticsParams = {}): Promise<WorklogAnalytics> => {
 ...
 * Export worklogs
 */
const exportWorklogs = async (params: WorklogListParams & { format: 'csv' | 'excel' | 'pdf' }): Promise<Blob> => {
...
export const useInfiniteWorklogs = (
  ...
  options?: Omit<UseInfiniteQueryOptions<WorklogListResponse, Error>, 'queryKey' | 'queryFn' | 'getNextPageParam'>
) => {
  return useInfiniteQuery({
    queryKey: queryKeys.worklogs.list(baseParams),
    queryFn: ({ pageParam = 1 }) => fetchWorklogs({ ...baseParams, page: pageParam as number }),
...
export const useUpdateWorklog = (
  options?: UseMutationOptions<JiraWorklog, Error, { id: string; data: UpdateWorklogData }>
) => {
  const queryClient = useQueryClient();
...
export const useExportWorklogs = (
  options?: UseMutationOptions<Blob, Error, WorklogListParams & { format: 'csv' | 'excel' | 'pdf' }>
) => {
  return useMutation({
 ...
 */
export const sortToParams = (sort: SortConfig): Pick<WorklogListParams, 'sortBy' | 'sortOrder'> => {
  return {
    sortBy: sort.field,
